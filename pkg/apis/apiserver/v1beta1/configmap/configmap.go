...
import (
	...
	api "server/pkg/apis/apiserver/v1beta1"
...
func CreateConfig(req *http.Request) (string, interface{}) {
	...
	result, err := api.CreateConfigMap(clusterID, configMap)
	if err != nil {
...
func AddConfigData(req *http.Request) (string, interface{}) {
	...
	namespace := params["namespace"]
	name := params["name"]
	configMap, err := api.GetConfigMapByName(name, namespace, clusterID)
	if err != nil {
		return httpx.StatusInternalServerError, err
	}
	for k, v := range data {
	...
	result, err := api.UpdateConfigMap(clusterID, configMap)
	if err != nil {
		return httpx.StatusInternalServerError, err
	}
	return httpx.StatusOK, result
...
func DeleteConfig(req *http.Request) (string, interface{}) {
	...
	namespace := params["namespace"]
	name := params["name"]
	err := api.DeleteConfigMap(name, namespace, clusterID)
	if err != nil {
		return httpx.StatusInternalServerError, err
	}
...
func ListConfig(req *http.Request) (string, interface{}) {
	...
	clusterID := params["clusterID"]
	namespace := params["namespace"]
	result, err := api.ListConfigMap(namespace, clusterID)
	if err != nil {
		return httpx.StatusInternalServerError, err
	}
	return httpx.StatusOK, result
