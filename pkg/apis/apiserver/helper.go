...
CREATE_DEPLOYMENT:
	resdeploy, err := fake.Deployments().CreateDeployment(deploy)
	if err != nil {
		if errors.IsConflict(err) {
...
DELETE_DEPLOYMENT:
	err = fake.Deployments().DeleteDeployment(name, namespace)
	if err != nil {
		if errors.IsConflict(err) {
...
func DeleteServiceByAppName(name, namespace, clusterID string) error {
	fake := k8s.GetClientset(clusterID)
	labels := fmt.Sprintf("%v=%v", apiserver.MinipaasAppName, name)
	if err := fake.Deployments().DeleteDeploymentByLabels(labels, namespace); err != nil {
		return err
	}
...
func ListServiceByAppName(name, namespace, clusterID string) ([]v1.Service, []v1beta1.Deployment, error) {
	...
	deployments, err := fake.Deployments().ListDeployment(labels, namespace)
	if err != nil {
		return []v1.Service{}, []v1beta1.Deployment{}, err
	}
...
func stopService(svc *v1.Service, deploy *v1beta1.Deployment, clusterID string) error {
	...
	if _, err := fake.Deployments().UpdateDeployment(deploy); err != nil {
		return err
	}
	return nil
}
...
func startService(svc *v1.Service, deploy *v1beta1.Deployment, clusterID string) error {
	fake := k8s.GetClientset(clusterID)
	deploy.Spec.Replicas = parseUtil.StringToInt32Pointer(deploy.ObjectMeta.Labels["replicas"])
	if _, err := fake.Deployments().UpdateDeployment(deploy); err != nil {
		return err
	}
	return nil
...
func ListService(namespace, clusterID string) ([]v1.Service, []v1beta1.Deployment, error) {
	...
	deployments, err := fake.Deployments().ListDeployment("", namespace)
	if err != nil {
		return []v1.Service{}, []v1beta1.Deployment{}, err
	}
	return services, deployments, nil
...
func GetPodEvents(name, namespace, clusterID string) ([]apiserver.Event, error) {
	fake := k8s.GetClientset(clusterID)
	events, err := fake.Events().GetEvents(namespace)
	if err != nil {
		return []apiserver.Event{}, err
	}
	var list []apiserver.Event
...
func GetServiceEvents(name, namespace, clusterID string) ([]apiserver.Event, error) {
	fake := k8s.GetClientset(clusterID)
	events, err := fake.Events().GetEvents(namespace)
	if err != nil {
		return []apiserver.Event{}, err
...
func GetDeployment(name, namespace, clusterID string) (*v1beta1.Deployment, error) {
	fake := k8s.GetClientset(clusterID)
	return fake.Deployments().GetDeployment(name, namespace)
}
...
func UpdateDeployment(deploy *v1beta1.Deployment, clusterID string) (*v1beta1.Deployment, error) {
	fake := k8s.GetClientset(clusterID)
	return fake.Deployments().UpdateDeployment(deploy)
}
...
func UpdateService(svc *v1.Service, deploy *v1beta1.Deployment, clusterID string) (*v1.Service, *v1beta1.Deployment, error) {
	fake := k8s.GetClientset(clusterID)
UPDATE_DEPLOYMENT:
	deployment, err := fake.Deployments().UpdateDeployment(deploy)
	if err != nil {
		if errors.IsConflict(err) {
...
func DeploySatefulService(service, headlessService *v1.Service, statefulset *v1beta1.StatefulSet, clusterID string) ([]interface{}, error) {
	...
	stateful, err := fake.Statefulsets().CreateStatefulSet(statefulset)
	if err != nil {
		go deleteK8SServices(service.Namespace, clusterID, service.Name, headlessService.Name)
		return result, err
	}
...
func getStatefulSet(name, namespace, clusterID string) (*v1beta1.StatefulSet, error) {
	fake := k8s.GetClientset(clusterID)
	return fake.Statefulsets().GetStatefulSet(name, namespace)
}
