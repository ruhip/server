...
type StatefulSetCache struct {
	sync.RWMutex
	List map[string]map[string]v1beta1.StatefulSet
}

type DeploymentCache struct {
	sync.RWMutex
	List map[string]map[string]v1beta1.Deployment
}
...
func init() {
	Store = &Cache{
		...
		&StatefulSetCache{
			List: make(map[string]map[string]v1beta1.StatefulSet, MAX_SIZE),
		},
		&DeploymentCache{
			List: make(map[string]map[string]v1beta1.Deployment, MAX_SIZE),
		},
		&ConfigMapCache{
				...
				for _, pod := range podlist {
					if svc.Name == pod.ObjectMeta.Labels["name"] {
						...
						if pod.Status.Phase == "Running" {
							container.Status = resources.AppRunning
						}
						if pod.Status.Phase == "Pending" {
							container.Status = resources.AppBuilding
						}
						if pod.Status.Phase == "Succeeded" {
							container.Status = resources.AppSuccessed
						}
						if pod.Status.Phase == "Failed" {
							container.Status = resources.AppFailed
						}
						if pod.Status.Phase == "Unknown" {
							container.Status = resources.AppUnknow
						}
...
func loop(param interface{}, nsname string) {
	...
	case *v1beta1.StatefulSetList:
		items := param.(*v1beta1.StatefulSetList).Items
		if len(items) == 0 {
			Store.StatefulSetCache.List[nsname] = make(map[string]v1beta1.StatefulSet)
		} else {
			Store.StatefulSetCache.Lock()
			stfpmap := make(map[string]v1beta1.StatefulSet)
	...
	case *v1beta1.DeploymentList:
		items := param.(*v1beta1.DeploymentList).Items
		if len(items) == 0 {
			Store.DeploymentCache.List[nsname] = make(map[string]v1beta1.Deployment)
		} else {
			Store.DeploymentCache.Lock()
			dpmap := make(map[string]v1beta1.Deployment)
